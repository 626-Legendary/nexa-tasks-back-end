...
const getTasks = async (req, res) => {
  try {
    ...
    // 查询任务，并关联assignedTo字段，取部分字段
    let tasks = await Task.find(filter).populate(
      "assignedTo",
      "name email profileImageUrl"
    );

    // 计算每个任务已完成的todoChecklist数量
    ...
    // 统计各状态任务数量
    const countFilterBase = isAdmin &&  adminOwnTasks !== "Y" ? {} : { assignedTo: req.user._id };

    const allTasks = await Task.countDocuments(countFilterBase);
...
const getUserDashboardData = async (req, res) => {
    try {
        ...
        //Fetech statistics for user-specific tasks
